...
{:ok, contact} = NodePing.Contats.get_by_id(token, contact_id)
...
contact_id = "IRAB6O5W"
{:ok, contact} = NodePing.Contats.get_by_id(token, parent_contact_id)
...
{:ok, updated} = NodePing.Contacts.update_contact(token, parent_contact_id, %{addresses: updated_addresses})
----
